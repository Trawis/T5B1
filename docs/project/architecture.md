...
- **Build configurations.** The solution defines exactly two configurations,
  ...
  (The Max Market Share button/toggle are the one exception: they are hidden
  from the UI and their implementation is commented out in
  `TrainerBehaviour.cs` rather than kept unconditional, because they called a
  `SoftwareProduct.MarketShare` member that does not exist on the currently
  vendored `Assembly-CSharp.dll`, confirmed by inspecting its metadata
  directly, so the assumption behind the earlier "compiles unconditionally"
  claim for that feature was wrong. See #150 for re-enabling it once a
  verified mechanism exists.)
